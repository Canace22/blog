...
JavaScript 的显式类型转换比较容易理解就是利用对应类型的构造函数去转换。
...
直接调用 Boolean 构造函数不传参数，返回的是 false，这里需要注意一下
...
- 调用 Boolean 构造函数不传参(这个不知道怎么归类，放到这里先)
...
Number 构造函数可以转换各种类型值为数值类型，parseInt()和 parseFloat() 专门用于把字符串转换成数值
