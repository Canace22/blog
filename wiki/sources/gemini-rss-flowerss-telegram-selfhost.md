...
- [来源：Gemini 对话——indes/flowerss-bot 项目原理](../sources/gemini-flowerss-bot-principle.md)（架构与依赖，与本会话互补）
- [来源：RSS 推送到 TG 的可行性探索](../sources/rss-telegram-selfhost.md)（Hexo 博文：`flowerss-bot`、Docker 代理与 Token 重置的一手记录）
- [docker 打包本地 web 项目发布到服务端](../sources/docker-documentation.md)（通用 Docker 发布思路）
