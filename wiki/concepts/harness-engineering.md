...
以 [Pinecone Nexus](../sources/pinecone-nexus-knowledge-engine.md) 为代表的路线，把 harness 的重点放在**知识编译层**：

- 用自主 Context Compiler 替代手工构建 artifact
- 编译器迭代优化 `curate()` 和 `query()` 两个函数，以 eval 驱动
- 声明式查询接口（KnowQL）让 agent 指定"需要什么"而非自己组装
- 将领域知识的结构发现、粒度选择、检索策略全部自动化

这一路线的价值在于：它把 context engineering 从"每个领域手工调"变成**自动基础设施**。
